# Iosevka Mayukai Firastorm

Custom Iosevka font build version 34.0.0 with minimal build.

## Build Guide

See [Iosevka Custom Build Documentation](https://github.com/be5invis/Iosevka/blob/main/doc/custom-build.md) for build instructions.

## Credits
- [Iosevka](https://github.com/be5invis/Iosevka) - Main font
- [Iosevka-Mayukai HowToBuild](https://github.com/Iosevka-Mayukai/HowToBuild) - Build guide

## License

This project uses the SIL Open Font License 1.1. See [License.md](License.md) for full license text.
